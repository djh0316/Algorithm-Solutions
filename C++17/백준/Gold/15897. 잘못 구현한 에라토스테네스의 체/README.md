...
이 문제는 겉보기엔 에라토스테네스의 체를 묻는 것 같지만, 실제로는 특정 수식의 합을 $O(\sqrt{n})$ 시간 복잡도 안에 해결해야 하는 **정수론** 문제.

* **구해야 하는 값**: 코드의 루프를 분석하면 결국  $\sum_{i=1}^{n} \lfloor \frac{n-1}{i} \rfloor$ 형태의 값을 구하는 문제.
* **문제점**: $n$이 최대 $10^9$이므로, 일반적인  반복문으로는 시간 초과(TLE)가 발생.
* **해결책**: 결과값은 i가 커질수록 일정 구간 동안 같은 값을 유지한다는 성질을 이용한 **평균값 정리 기반의 최적화(Square Root Decomposition)**를 통해 시간 복잡도를 최소.
...
* i 가 부터 $\sqrt{n-1}$까지는 값이 급격히 변하므로 일일이 더한다.
* i 가 $\sqrt{n-1}$보다 커지면  값이 중복되는 구간이 길어진다.
...
* **데이터 타입 오류**: n의 범위가 크고 합산 결과가 매우 크므로 `int`를 사용하면 **Overflow**가 발생하므로 반드시 `long long`을 사용해야 한다.
* **경계값 처리**: n = 1인 경우나 루프의 마지막 인덱스에서 나누기 0이 발생하지 않도록 예외 처리한다.
* **수식의 정확성**: 단순히 $\frac{n}{i}$가 아니라 문제 코드의 로직에 따라 `(n-1)/i`가 적용되는 범위를 정확히 산출되도록 한다.

---

## 4. 복기 및 학습 내용 (Retrospective)

* **정수론적 접근**: $O(n)$의 벽을 느꼈을 때, 수열의 규칙성을 찾아 $\sqrt{n}$으로 줄이는 사고방식을 연습할 수 있었다.
* **코드 효율성**: 작성한 코드에서 `sqrt(n)`을 변수로 선언하여 반복적인 연산을 피했다.
* **느낀 점 및 배운점**: 골드 2 문제의 난이도, 정수론 문제를 처음 풀어보는데 공식에 대한 재해석과 반복문의 n 범위를 2, 3, 4 ... 으로 나눠 보며 점차적으로 sqrt(n)의 값으로 줄인 결과 문제를 해결 할 수 있었다.
